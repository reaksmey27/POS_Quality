    ...
    <div v-else class="space-y-3 max-h-[260px] overflow-y-auto pr-1">
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="w-9 h-9 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-xs border border-indigo-100/30 dark:border-none"
          >
            #{{ order.id }}
          </div>
          <div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="w-24 text-right">
            ...
            <p
              class="text-[10px] font-bold text-emerald-600 dark:text-emerald-500"
            >
...
const formatRiel = (usdValue) =>
  (parseFloat(usdValue || 0) * 4100).toLocaleString();
