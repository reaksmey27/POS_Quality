<template>
  <div
    class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden flex flex-col h-[calc(100vh-12rem)] sticky top-6"
  >
    <div
      class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/20"
    >
    ...
    <div
      class="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60 px-5"
    >
      <div
        v-if="cart.length === 0"
      ...
      >
        <ShoppingBagIcon
          class="w-8 h-8 text-slate-300 dark:text-slate-700 mb-2"
        />
        <h5 class="text-xs font-bold text-slate-700 dark:text-slate-400">
          Basket is empty
        </h5>
        <p
          class="text-[11px] text-slate-400 dark:text-slate-500 max-w-[180px] mt-0.5"
        >
      ...
      >
        <div class="min-w-0 flex-1 flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-200/60 dark:border-slate-800 flex items-center justify-center overflow-hidden shrink-0"
          >
            <img
              v-if="item.image"
              :src="item.image"
          ...
          <div class="min-w-0">
            ...
            <span
              class="text-xs font-medium text-slate-400 dark:text-slate-500 mt-0.5 block"
            >
        ...
        <div class="flex items-center gap-1 shrink-0">
          ...
          <span
            class="w-7 text-center font-mono text-xs font-bold text-slate-900 dark:text-white"
          >
            {{ item.qty }}
          </span>
          <button
    ...
    <div
      class="p-5 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 space-y-4 shrink-0"
    >
      <div class="space-y-1.5">
        ...
        <div
          class="flex items-center justify-between text-sm font-bold text-slate-950 dark:text-white pt-1.5 border-t border-dashed border-slate-200 dark:border-slate-800"
        >
...
defineEmits(["increase", "decrease", "clear", "checkout"]);
</script>
