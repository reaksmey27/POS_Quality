    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all animate-in scale-in duration-150">
      ...
      <div class="p-5 space-y-5">
        ...
        <div class="bg-indigo-50/60 dark:bg-indigo-950/20 p-4 rounded-xl border border-indigo-100/70 dark:border-indigo-900/30 text-center">
          ...
          <span class="text-3xl font-black text-indigo-900 dark:text-indigo-300 font-mono block mt-1">
        ...
        >
          <label for="cash-tendered-input" class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">
            Amount Tendered
          </label>
          <div class="relative">
        ...
        >
          <div class="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl relative overflow-hidden shadow-xs">
            <div class="absolute inset-x-0 h-0.5 bg-indigo-500 shadow-md shadow-indigo-400 top-0 animate-bounce"></div>
            <QrCodeIcon class="w-24 h-24 text-slate-800 dark:text-slate-200 opacity-90" />
          </div>
          <span class="text-[11px] font-medium text-slate-400 dark:text-slate-500 mt-2.5 animate-pulse">
            Awaiting terminal customer capture...
          </span>
        </div>
      </div>
...
const changeDue = computed(() => {
  const cash = parseFloat(amountTendered.value) || 0;
  if (!cash) return -props.total;

  return Math.round((cash - props.total) * 100) / 100;
});

const isSubmitDisabled = computed(() => {
  if (props.checkoutLoading) return true;
  if (paymentMethod.value === "cash" && changeDue.value < 0) return true;
  return false;
});
...
const handleSubmit = () => {
  ...
  emit("confirm-checkout", {
    ...
    tendered: paymentMethod.value === "cash" ? parseFloat(amountTendered.value) || 0 : props.total
  });
};
</script>