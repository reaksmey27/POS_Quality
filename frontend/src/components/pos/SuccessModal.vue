  ...
  >
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl w-full max-w-sm p-6 text-center transform transition-all animate-in scale-in duration-150">
      ...
      <div class="my-5 p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800/80 text-center space-y-3">
        <div>
          <span class="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider block">
            Total Amount Paid
          </span>
        ...
        <div class="pt-2 border-t border-slate-200/60 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 font-medium">
          <span>Channel</span>
          <span class="capitalize font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5 select-none">
            <component 
              :is="paymentChannelIcon" 
...
const parsedTotalAmount = computed(() => {
  const rawValue = props.order.total_price ?? props.order.total ?? 0;
  const numericValue = parseFloat(rawValue);
  return Number.isFinite(numericValue) ? numericValue.toFixed(2) : "0.00";
});

const paymentChannelIcon = computed(() => {
  const cleanMethod = props.order.payment_method?.trim().toLowerCase();
  return cleanMethod === 'qr' ? QrCodeIcon : BanknoteIcon;
});
