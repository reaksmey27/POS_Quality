<template>
  <div
    class="p-6 max-w-7xl mx-auto space-y-6 text-slate-900 dark:text-slate-100 transition-colors duration-150"
  >
    <div
      class="border-b border-slate-100 dark:border-slate-800 pb-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
    >
      <div>
        <h1
      ...
      <div
        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 shadow-sm text-xs font-semibold flex items-center gap-2 select-none"
      >
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5" :class="gridSpanClass">
      ...
      >
        ...
        <div
          class="w-12 h-12 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center select-none"
        >
      ...
      >
        ...
        <div
          class="w-12 h-12 rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400 flex items-center justify-center select-none"
        >
      ...
      >
        ...
        <div
          class="w-12 h-12 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center select-none"
        >
      ...
      >
        <div class="space-y-0.5">
          ...
          <p
            class="text-[11px] font-bold text-emerald-600 dark:text-emerald-500 tracking-wide"
          >
    ...
    >
      <div
        class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 select-none"
      >
        <h2 class="font-semibold text-slate-700 dark:text-slate-300 text-sm">
          Quick Actions
        </h2>
      </div>
      <div class="p-6 flex flex-wrap items-center gap-3">
        <RouterLink
          to="/pos"
          class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg shadow-sm transition-colors cursor-pointer"
        >
        ...
        <RouterLink
          v-if="canManage"
          to="/products"
          class="inline-flex items-center gap-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-medium px-4 py-2.5 rounded-lg shadow-sm transition-colors cursor-pointer"
        >
...
const fetchDashboardStatsOnce = async () => {
  ...
  if (canManage.value) {
    ...
    stats.value.revenue = orderList.reduce(
...
const fetchRecentCompleted = async () => {
  const res = await api.get("/orders/recent", { params: { limit: 10 } });
  const list = res.data?.data || [];
  recentOrders.value = list.slice(-4).reverse();
};

onMounted(async () => {
  try {
    await fetchDashboardStatsOnce();

    pollTimer = setInterval(async () => {
      try {
        await fetchRecentCompleted();
      } catch (err) {
        console.error("Failed to refresh recent transactions:", err);
      }
    }, 5000);
