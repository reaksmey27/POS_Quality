<template>
  <div class="p-6 max-w-7xl mx-auto space-y-6 text-slate-800 dark:text-slate-100 transition-colors duration-150">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
      ...
      <div>
        <button
          ...
          type="button"
          class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-colors duration-150 cursor-pointer"
        >
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
      ...
      >
        <div class="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 mb-3">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full text-left border-collapse text-sm">
          <thead>
            <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/20 text-slate-600 dark:text-slate-400 font-semibold">
      ...
      <div
        ...
        class="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/30 dark:bg-slate-800/10"
      >
        ...
        <div class="inline-flex items-center gap-1.5">
          <button
            ...
            type="button"
            class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40 disabled:hover:bg-white dark:disabled:hover:bg-slate-800 cursor-pointer disabled:cursor-not-allowed transition-colors"
          >
          ...
          <button
            type="button"
            class="w-8 h-8 text-xs font-bold text-white bg-indigo-600 rounded-lg pointer-events-none shadow-sm"
          >
            {{ currentPage }}
          </button>
...
const handleSave = async (formData) => {
  if (!formData.name || !formData.price) {
    modalError.value = "Name and price are required";
    return;
  }
...
const loadInventory = async () => {
  ...
  try {
    ...
    console.error("An error occurred during item initialization: ", err);
  } finally {
    loading.value = false;
  }
};
